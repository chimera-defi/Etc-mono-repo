...
AgenticUSG is a productive stablecoin system for ETHGlobal Cannes 2026: users deposit yield-bearing collateral, mint `aUSG`, and opt into risk tranches with explicit liquidity commitments while a human-backed AI agent rotates collateral into the best available yield source, including near-resolution prediction-market arbitrage.
...
- tranche ladder:
  - senior = lower yield, faster exit, redemption priority
  - mezz = medium lock and medium loss absorption
  - junior = first-loss capital, longest lock or ve-style escrow, highest upside
...
4. Show tranche split across senior, mezz, junior with lockup and redemption differences.
...
5. A mild peg stress event shows why junior capital is the sticky first-loss buffer and why senior exits are more conservative.
...
6. Junior lockups or ve-style escrow should be real in the product story, even if implemented as simple lock metadata in MVP.
