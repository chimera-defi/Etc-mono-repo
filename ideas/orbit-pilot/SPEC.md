...
9. **V1** optional extras: `orbit-pilot[browser]` (Playwright) for portal assist; `python -m orbit_pilot` equals `orbit` CLI; **`orbit schedule-*`** for deferred subprocess jobs; **`orbit registry-lint`** validates platform registry YAML; **`orbit pipeline`** runs plan+doctor+generate+check-run in one `--json` payload; **`orbit init --preset walletradar`** copies WalletRadar-shaped launch stub.
