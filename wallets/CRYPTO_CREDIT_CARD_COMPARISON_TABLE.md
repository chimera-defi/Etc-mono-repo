# Crypto Credit Card Comparison Table

> **Data Status:** ✅ Expanded to 26 cards (December 2025). Cards grouped by provider for consistent scoring. Verify all information on official websites before applying.

**Last Updated:** December 2025

## Complete Card Comparison

| Card | Score | Type | Cash Back | Annual Fee | Foreign Fee | Crypto Rewards | Staking Req | Provider | Business | Website | Status | Best For | Notes |
|------|-------|------|-----------|------------|-------------|----------------|-------------|----------|----------|---------|--------|----------|-------|
| **Bybit Card** | 88 🟢 | Debit | Up to 10% | $0 | 0% | Multi-crypto | None | Bybit Exchange | ✅ | [bybit.com/cards](https://www.bybit.com/en/cards/) | ✅ Active | High cashback | EEA/Switzerland, up to 10% cashback |
| **Mode Card** | 87 🟢 | Debit | Up to 10% | $0 | 0% | BTC | None | Mode | ❌ | [modeapp.com](https://modeapp.com) | ⚠️ Verify | Bitcoin rewards | ⚠️ UK-based - verify availability |
| **Hi Card** | 86 🟢 | Debit | Up to 10% | $0 | 0% | HI | Tiered | hi.com | ❌ | [hi.com](https://hi.com) | ✅ Active | High cashback | Web3 neobank, up to 10% cashback |
| **Plutus Card** | 85 🟢 | Debit | 3-9% | £0 | 0% | PLU | Optional | Plutus | ❌ | [plutus.it](https://plutus.it) | ✅ Active | EU users | ✅ Verified: 3% base, up to 9% via tiers |
| **Coinbase Card** | 82 🟢 | Debit | 1-4% | $0 | 0% | BTC, ETH, others | None | Coinbase Exchange | ❌ | [coinbase.com/card](https://www.coinbase.com/card) | ✅ Active | Coinbase users | -3 pts exchange risk |
| **Nexo Card** | 82 🟢 | Credit | Up to 2% | $0 | 0% | NEXO | None | Nexo | ✅ | [nexo.com/card](https://nexo.com/card) | ✅ Active | Crypto credit | ✅ Verified: dual-mode Debit + Credit |
| **Binance Card** | 79 🟢 | Debit | Up to 8% | $0 | 0% | BNB | None | Binance Exchange | ⚠️ Verify | [binance.com/en/cards](https://www.binance.com/en/cards) | ✅ Active | Binance users | -3 pts exchange risk |
| **Wirex Card** | 78 🟢 | Debit | Up to 8% | $0 | 0% | WXT | None | Wirex | ⚠️ Verify | [wirexapp.com/card](https://wirexapp.com/card) | ✅ Active | Multi-crypto | Own infrastructure |
| **Gnosis Pay** | 78 🟢 | Debit | Up to 5% | €0 | 0% | GNO/SAFE | None | Gnosis DAO | ❌ | [gnosispay.com](https://gnosispay.com) | ✅ Active | DeFi-native | Non-custodial, EU/UK |
| **Fold Card** | 76 🟢 | Debit | Up to 5% | $0 | 0% | BTC | None | Fold | ❌ | [foldapp.com](https://foldapp.com) | ✅ Active | Bitcoin rewards | US only |
| **1inch Card** | 75 🟢 | Debit | Up to 2% | $0 | 0% | 1INCH | None | Baanx (1inch) | ❌ | [1inch.com/card](https://1inch.com/card) | ✅ Active | DeFi users | Baanx + Mastercard powered |
| **Gemini Credit Card** | 74 🟡 | Credit | 1-3% | $0 | 0% | BTC, ETH | None | Gemini Exchange | ❌ | [gemini.com/credit-card](https://www.gemini.com/credit-card) | ✅ Active | Credit building | -3 pts exchange risk |
| **KuCard** | 72 🟡 | Debit | Up to 4% | $0 | 0% | Multi-crypto | None | KuCoin Exchange | ❌ | [kucoin.com/kucard](https://www.kucoin.com/kucard) | ✅ Active | KuCoin users | -3 pts exchange risk |
| **Revolut Crypto** | 72 🟡 | Debit | 1-3% | $0/paid | 0% | Multi-crypto | None | Revolut | ✅ | [revolut.com](https://www.revolut.com/crypto/crypto-card/) | ✅ Active | Fiat+Crypto | Established fintech |
| **Shakepay Card** | 70 🟡 | Debit | Up to 1.5% | $0 | 0% | BTC | None | Shakepay | ❌ | [shakepay.com/card](https://shakepay.com/card) | ✅ Active | Canada | ✅ Verified: Canada only |
| **Redotpay** | 70 🟡 | Debit | Up to 3% | $0-$100 | 1-2% | USDT, USDC | None | Redotpay | ✅ | [redotpay.com](https://redotpay.com) | ✅ Active | Multi-region | Hong Kong-based, global |
| **OKX Card** | 68 🟡 | Debit | TBD | $0 | TBD | Stablecoins | None | OKX Exchange | ⚠️ Verify | [okx.com](https://www.okx.com) | 🔄 Launching | Stablecoins | New Mastercard partnership |
| **Uphold Card** | 67 🟡 | Debit | 1-4% | $0 | 0% | XRP, BTC, ETH | None | Uphold | ⚠️ Verify | [uphold.com/card](https://uphold.com/card) | ⚠️ Verify | Multi-crypto | ⚠️ Website 404 - verify |
| **CryptoSpend Card** | 62 🟡 | Debit | 1% | $0 | 0% | BTC, ETH, others | None | CryptoSpend | ❌ | [cryptospend.com.au](https://cryptospend.com.au) | ✅ Active | Australia | Australia only |
| **CoinJar Card** | 60 🟡 | Debit | 1% | $0 | 0% | BTC | None | CoinJar Exchange | ❌ | [coinjar.com/card](https://www.coinjar.com/card) | ⚠️ Verify | Australia | ⚠️ Website 404 - verify |
| **Kraken Card** | 60 🟡 | Debit | TBD | $0 | TBD | Multi-crypto | None | Kraken Exchange | ⚠️ Verify | [kraken.com](https://www.kraken.com) | 🔄 Limited | Kraken users | Expanding availability |
| **Crypto.com Visa** | 57 🟡 | Debit | 0.5-8% | $0 | 0% | CRO | $0-$400k+ | Crypto.com Exchange | ✅ | [crypto.com/cards](https://crypto.com/cards) | ✅ Active | CRO holders | ⚠️ -23 pts: -20 UX, -3 exchange |
| **Swissborg Card** | 56 🟡 | Debit | 1-2% | $0 | 0% | CHSB | None | Swissborg | ❌ | [swissborg.com/card](https://swissborg.com/card) | ⚠️ Verify | EU users | ⚠️ Website 404 - verify |
| **Reap** | 55 🟡 | Business Credit | 0% | $0 | 2% | Stablecoins | 1:1 Collateral | Reap | ✅ | [reap.global](https://reap.global) | ✅ Active | Business | ✅ Verified: Corporate Visa |
| **BitPay Card** | 52 🟡 | Prepaid | 0% | $0 | 0% | None | None | BitPay | ⚠️ Verify | [bitpay.com/card](https://bitpay.com/card) | ✅ Active | Bitcoin spending | Prepaid, no rewards |
| **EtherFi Cash** | 50 🟡 | Credit | TBD | TBD | TBD | ETH, BTC, USDC | TBD | EtherFi | ✅ | [ether.fi/cash](https://ether.fi/cash) | ✅ Active | DeFi users | Non-custodial, rates TBD |
| **BlockFi Rewards** | - | Credit | 1.5% | $0 | 0% | BTC | None | BlockFi | - | - | ❌ Discontinued | - | Bankruptcy 2022 |
| **MCO Card** | - | Credit | 1-5% | $0 | 0% | MCO | $0-$50k+ | Crypto.com | - | - | ❌ Discontinued | - | Replaced by CRO card |
| **Monolith Card** | - | Debit | - | - | - | ETH | - | Monolith | - | - | ❌ Discontinued | - | Ethereum card, inactive |

**Legend:**
- **Score:** 0-100 weighted score (see [Scoring Methodology](./CRYPTO_CREDIT_CARD_COMPARISON_DETAILS.md#scoring-methodology))
  - 🟢 **75+:** Highly Recommended
  - 🟡 **50-74:** Good Option
  - 🔴 **<50:** Consider Alternatives
- **Type:** Credit card vs. Debit card vs. Prepaid vs. Business
- **Cash Back:** Base cash back percentage (may vary by category)
- **Annual Fee:** $0 or annual fee amount
- **Foreign Fee:** Foreign transaction fee percentage
- **Crypto Rewards:** Additional crypto rewards or reward currency
- **Staking Req:** Minimum crypto staking/balance required for benefits
- **Provider:** Card issuer/exchange - cards from same provider share scoring handicaps
- **Business:** ✅ Business accounts supported | ❌ Personal only | ⚠️ Verify (needs verification)
- **Website:** Official card website link
- **Status:** ✅ Active | ⚠️ Verify | 🔄 Launching/Limited | ❌ Discontinued

**⚠️ Verification Note:** 
- Cards marked with "⚠️ Verify" have website issues or need manual verification
- Some information may have changed - verify on official websites
- See [VERIFICATION_NOTES.md](./VERIFICATION_NOTES.md) for detailed verification status

**📊 Scoring Note:**
- Scores include provider-based handicaps for consistent evaluation
- Exchange-based cards receive **-3 points** (custody risk)
- **Crypto.com receives additional -20 points** (poor UX, customer service, staking lock-ups)
- See [CRYPTO_CREDIT_CARD_COMPARISON_DETAILS.md](./CRYPTO_CREDIT_CARD_COMPARISON_DETAILS.md#score-summary) for breakdowns

---

## Scoring Methodology

**Quick Reference:** Score = Cash Back (30) + Fees (20) + Crypto Rewards (15) + Availability (10) + Staking Req (10) + Card Type (10) + Trust (5) + Provider Handicaps (-23 to +2) = 100 total

### Provider-Based Handicaps

Cards from the same provider/exchange category receive consistent scoring adjustments:

| Category | Handicap | Cards Affected | Rationale |
|----------|----------|----------------|-----------|
| **Exchange-Based** | -3 pts | Coinbase, Binance, Gemini, KuCoin, OKX, CoinJar, Kraken, Crypto.com, Bybit | Custody risk, platform dependency |
| **Crypto.com Specific** | -20 pts | Crypto.com Visa | Poor UX, customer service, staking lock-ups |
| **404/Verify Issues** | -5 pts | Swissborg, Uphold, CoinJar | Website issues, unclear status |
| **DeFi-Native** | +0 pts | Gnosis Pay, 1inch Card, EtherFi | Non-custodial but complex |
| **Verified Cards** | +2 pts | Plutus, Nexo, Shakepay, Reap | Confirmed active and accurate |

### Baanx Infrastructure Note

**Baanx** is a white-label crypto card provider powering multiple cards:
- **1inch Card** - Baanx + Mastercard partnership
- **Upcoming Tezos Card** - Non-custodial on-chain
- Various USDC stablecoin cards with Visa

Cards powered by Baanx share underlying infrastructure but are scored independently based on their features.

See [CRYPTO_CREDIT_CARD_COMPARISON_DETAILS.md](./CRYPTO_CREDIT_CARD_COMPARISON_DETAILS.md#scoring-methodology) for detailed methodology.

---

## Detailed Card Information

### Active Cards

#### Coinbase Card
- **Type:** Debit card
- **Cash Back:** 1% base, up to 4% with Coinbase One subscription
- **Rewards:** BTC, ETH, and other supported cryptocurrencies
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [coinbase.com/card](https://www.coinbase.com/card)
- **Availability:** US only
- **Notes:** Instant crypto rewards, no credit check

#### Crypto.com Visa Card
- **Type:** Debit card (prepaid)
- **Cash Back:** Tiered 0.5% to 8% based on CRO stake
  - Midnight Blue: 0.5% (no stake)
  - Ruby Steel: 2% ($400 CRO stake)
  - Royal Indigo/Jade Green: 3% ($4,000 CRO stake)
  - Frosted Rose Gold/Icy White: 5% ($40,000 CRO stake)
  - Obsidian: 8% ($400,000 CRO stake)
- **Rewards:** CRO token
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** $0-$400,000+ CRO
- **Business Support:** ✅ Yes (Crypto.com Business accounts available)
- **Website:** [crypto.com/cards](https://crypto.com/cards)
- **Availability:** Global (varies by region)
- **Notes:** 6-month staking lock, additional perks at higher tiers

#### Gemini Credit Card
- **Type:** Credit card
- **Cash Back:** 1% base, up to 3% in specific categories
- **Rewards:** BTC or ETH (user choice)
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [gemini.com/credit-card](https://www.gemini.com/credit-card)
- **Availability:** US only
- **Notes:** Credit card (not debit), requires credit check

#### Nexo Card
- **Type:** Credit card (crypto-backed credit line)
- **Cash Back:** 0-2% cashback
- **Rewards:** NEXO token (up to 2% additional)
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None (but NEXO holders get better rates)
- **Business Support:** ✅ Yes (Nexo Business accounts available)
- **Website:** [nexo.com/card](https://nexo.com/card)
- **Availability:** EU, UK
- **Notes:** Uses crypto as collateral for credit line

#### Binance Card
- **Type:** Debit card
- **Cash Back:** Up to 8% (varies by BNB holdings)
- **Rewards:** BNB
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None (but BNB holdings affect rates)
- **Business Support:** ✅ Yes (Binance Business accounts available)
- **Website:** [binance.com/en/cards](https://www.binance.com/en/cards)
- **Availability:** Global (varies by region)
- **Notes:** Debit card, instant crypto-to-fiat conversion

#### Wirex Card
- **Type:** Debit card
- **Cash Back:** Up to 8% (Cryptoback™ rewards)
- **Rewards:** WXT token
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ✅ Yes (Wirex Business accounts available)
- **Website:** [wirexapp.com/card](https://wirexapp.com/card)
- **Availability:** Global (varies by region)
- **Notes:** Multi-crypto support, Cryptoback™ rewards program

#### Plutus Card
- **Type:** Debit card
- **Cash Back:** 3% base
- **Rewards:** PLU token
- **Annual Fee:** £0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** Optional (for additional perks)
- **Business Support:** ❌ Personal only
- **Website:** [plutus.it](https://plutus.it)
- **Availability:** EU, UK
- **Notes:** EU/UK only, PLU token rewards

#### Swissborg Card
- **Type:** Debit card
- **Cash Back:** 1-2%
- **Rewards:** CHSB token
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [swissborg.com/card](https://swissborg.com/card)
- **Availability:** EU
- **Notes:** EU only, Swissborg ecosystem integration

#### BitPay Card
- **Type:** Prepaid debit card
- **Cash Back:** 0%
- **Rewards:** None
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ✅ Yes (BitPay Business accounts available)
- **Website:** [bitpay.com/card](https://bitpay.com/card)
- **Availability:** US
- **Notes:** Bitcoin-focused, prepaid card

#### Fold Card
- **Type:** Debit card
- **Cash Back:** Up to 5% (in Bitcoin)
- **Rewards:** BTC only
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [foldapp.com](https://foldapp.com)
- **Availability:** US
- **Notes:** Bitcoin rewards only, rotating categories

#### Mode Card
- **Type:** Debit card
- **Cash Back:** Up to 10% (in Bitcoin)
- **Rewards:** BTC only
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [modeapp.com](https://modeapp.com)
- **Availability:** US
- **Notes:** Bitcoin rewards, high cash back rates

#### Shakepay Card
- **Type:** Debit card
- **Cash Back:** 1-2% (in Bitcoin)
- **Rewards:** BTC only
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [shakepay.com/card](https://shakepay.com/card)
- **Availability:** Canada only
- **Notes:** Canada only, Bitcoin rewards

---

### Business/Corporate Cards

**Note:** These cards are specifically designed for business/corporate use or offer dedicated business account options.

#### Reap
- **Type:** Business credit card (Corporate Visa card)
- **Website:** [reap.global](https://reap.global)
- **Pricing Page:** [reap.global/resources/info/pricing](https://reap.global/resources/info/pricing) ✅ Verified
- **Status:** ✅ Active (✅ Verified December 2025 via browser automation)
- **Business Support:** ✅ Yes (Business-focused platform - business accounts only)
- **Cash Back:** 0% (No cashback rewards)
- **Annual Fee:** $0 ✅ Verified (FREE)
- **Foreign Transaction Fee:** 2% ✅ Verified (FX fee: 2%)
- **ATM Fee:** 2% ✅ Verified
- **Crypto Rewards:** Stablecoin repayment option ✅ Verified
- **Staking Required:** 1:1 Collateral ratio ✅ Verified (Credit limit = 100% of collateral)
- **Card Type:** Physical & Virtual ✅ Verified (FREE)
- **Monthly Bill Repayment:** FREE (Fiat currency or Stablecoins) ✅ Verified
- **Available Currencies:** USD, HKD ✅ Verified
- **Mobile App:** FREE ✅ Verified
- **Availability:** Global (Hong Kong, Singapore, Mexico, Brazil - varies by region)
- **Notes:** ✅ **Verified December 2025** - Corporate Visa credit card with flexible bill repayment in fiat or digital currencies. Secured corporate credit card with advanced spend control, easy access, and additional perks. Late fees may apply. See [VERIFICATION_NOTES.md](./VERIFICATION_NOTES.md) for verification details.

#### EtherFi Cash
- **Type:** Credit card (DeFi-native)
- **Website:** [ether.fi/cash](https://ether.fi/cash)
- **Corporate Cards:** [ether.fi/corporate-cards](https://ether.fi/corporate-cards) ✅ Verified
- **Status:** ✅ Active (✅ Verified December 2025)
- **Business Support:** ✅ Yes (Corporate cards available for businesses)
- **Cash Back:** Cashback rewards (specific rates TBD - pending verification)
- **Annual Fee:** TBD (pending verification)
- **Foreign Transaction Fee:** TBD (pending verification)
- **Crypto Rewards:** ETH, BTC, Stablecoins (can use as collateral)
- **Staking Required:** TBD (likely optional)
- **Card Type:** Credit card (non-custodial, DeFi-native)
- **Availability:** Global (varies by region)
- **Notes:** ✅ **Verified** - DeFi-native crypto credit card available for individuals, families, and businesses. Corporate cards available. Use crypto without selling it. Specific rates and fees need deeper verification due to JavaScript-heavy page rendering. See [VERIFICATION_NOTES.md](./VERIFICATION_NOTES.md) for verification details.

---

### Other Active Cards (Personal & Business Options)

#### Uphold Card
- **Type:** Debit card
- **Cash Back:** 1-4% (varies by card tier)
- **Rewards:** XRP, BTC, ETH, and other supported cryptocurrencies
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ✅ Yes (Uphold Business accounts available)
- **Website:** [uphold.com/card](https://uphold.com/card)
- **Availability:** US, UK, EU (varies by region)
- **Notes:** Multi-crypto rewards, debit card

#### CoinJar Card
- **Type:** Debit card
- **Cash Back:** 1% (in Bitcoin)
- **Rewards:** BTC only
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [coinjar.com/card](https://www.coinjar.com/card)
- **Availability:** Australia only
- **Notes:** Australia only, Bitcoin rewards

#### CryptoSpend Card
- **Type:** Debit card
- **Cash Back:** 1% (in crypto)
- **Rewards:** BTC, ETH, and other supported cryptocurrencies
- **Annual Fee:** $0
- **Foreign Transaction Fee:** 0%
- **Staking Required:** None
- **Business Support:** ❌ Personal only
- **Website:** [cryptospend.com.au](https://cryptospend.com.au)
- **Availability:** Australia only
- **Notes:** Australia only, multi-crypto rewards

---

### Discontinued Cards

#### BlockFi Rewards Visa
- **Status:** ❌ Discontinued (2022)
- **Reason:** BlockFi filed for bankruptcy
- **Previous Terms:** 1.5% cash back in BTC, $0 annual fee

#### MCO Card (Crypto.com Legacy)
- **Status:** ❌ Discontinued
- **Reason:** Replaced by CRO-based card
- **Previous Terms:** Tiered rewards based on MCO stake

---

## Comparison Summary

### Top Scored Cards (🟢 75+ Highly Recommended)
1. **Mode Card** - 90 🟢 ⚠️ - Up to 10% Bitcoin rewards (UK/EU - verify)
2. **Plutus Card** - 88 🟢 - 3-9% PLU rewards (EU/UK)
3. **Coinbase Card** - 85 🟢 - 1-4% multi-crypto rewards (US)
4. **Nexo Card** - 84 🟢 - Up to 2% crypto-backed credit (EU/UK)
5. **Binance Card** - 82 🟢 - Up to 8% BNB rewards (Global)
6. **Wirex Card** - 80 🟢 - Up to 8% WXT rewards (Global)
7. **Fold Card** - 78 🟢 - Up to 5% Bitcoin rewards (US)
8. **Gemini Credit Card** - 75 🟢 - 1-3% credit card (US)

### Best Cash Back Rates
1. **Mode Card** - Up to 10% (Bitcoin)
2. **Crypto.com Obsidian** - 8% (with $400k+ CRO stake) ⚠️ Score: 59 🟡 (UX penalty: -20 pts)
3. **Binance Card** - Up to 8% (with BNB holdings)
4. **Wirex Card** - Up to 8% (WXT rewards)
5. **Fold Card** - Up to 5% (Bitcoin)

### No Annual Fee Cards
- Most crypto cards have $0 annual fees
- Notable exception: Some premium tiers may have fees (verify per card)

### Geographic Availability
- **US Only:** Coinbase, Gemini, BitPay, Fold
- **UK/EU (verify):** Mode
- **US, UK, EU:** Uphold (varies by region)
- **EU/UK:** Nexo, Plutus, Swissborg
- **Global:** Crypto.com, Binance, Wirex (varies by region)
- **Canada:** Shakepay
- **Australia:** CoinJar, CryptoSpend

### Business Account Support

**Cards with Business Account Support (Verified):**
- ✅ **Nexo Card** - Score: 84 🟢 - Business accounts available
- ✅ **Reap** - Score: 55 🟡 - Business-focused platform (business accounts only, verified)
- ✅ **EtherFi Cash** - Score: 50 🟡 - Corporate cards available (verified)
- ✅ **Crypto.com Visa** - Score: 59 🟡 - Business accounts available (⚠️ UX penalty: -20 pts)

**Cards Needing Verification for Business Support:**
- ⚠️ **Binance Card** - Score: 82 🟢 - Verify business account availability
- ⚠️ **Wirex Card** - Score: 80 🟢 - Verify business account availability
- ⚠️ **BitPay Card** - Score: 52 🟡 - Verify business account availability
- ⚠️ **Uphold Card** - Score: 70 🟡 - Verify business account availability (404 error)

**Cards with Personal Only (Verified):**
- ❌ Coinbase Card
- ❌ Gemini Credit Card
- ❌ Plutus Card
- ❌ Swissborg Card
- ❌ Fold Card
- ❌ Mode Card
- ❌ Shakepay Card
- ❌ CoinJar Card
- ❌ CryptoSpend Card

**⚠️ Note:** Business account support marked with ⚠️ needs manual verification on official websites. Some information may be based on assumptions. Please verify before applying for business accounts.

### Card Types
- **Credit Cards:** Gemini, Nexo (crypto-backed)
- **Debit Cards:** Most crypto cards (Coinbase, Crypto.com, Binance, Wirex, Uphold, etc.)
- **Prepaid:** BitPay

---

## Important Notes

⚠️ **Verify Before Applying:**
- All information should be verified on official card websites
- Terms and conditions change frequently
- Availability varies by jurisdiction
- Credit cards require credit checks (Gemini, Nexo)
- Debit cards may require account funding

⚠️ **Regulatory Considerations:**
- Crypto card regulations vary by country
- Some cards may not be available in all jurisdictions
- Tax implications vary by country

⚠️ **Staking Requirements:**
- Some cards require staking tokens (Crypto.com CRO)
- Staking may have lock-up periods
- Token values fluctuate (staking risk)

---

*Last updated: December 2025 - Core research complete. Scoring system implemented with Human Suggestions adjustments.*
