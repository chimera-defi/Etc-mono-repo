...
import { getAllWalletData, getWalletById, getWalletsByType, type WalletType } from '@/lib/wallet-data';
...
export async function generateMetadata({ params }: { params: { type: WalletType; id: string } }): Promise<Metadata> {
  ...
  const description = getWalletDescription(params.type, wallet);
  const title = getWalletMetaTitle(params.type, wallet);
  const ogImageUrl = `${baseUrl}/og-image.png?${ogImageVersion}`;
